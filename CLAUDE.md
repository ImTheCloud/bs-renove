...
- **Formulaires en étapes** : devis (2 étapes) et candidature (4 étapes : métier, expérience, sur le chantier, coordonnées) partagent **le même moteur** (`src/scripts/formulaire-etapes.ts`) et **le même style** (`src/styles/formulaire.css`) : ils doivent rester identiques. La page Rejoindre l'équipe est construite comme la page Contact (`SectionDevis formulaire="candidature"`). Après l'envoi : photos/CV par WhatsApp ou email, demande déjà écrite. Une case `data-precision` fait apparaître un champ (« Autre métier » → « Quel autre métier ? »). Chaque métier de la page Services a un bouton « Rejoindre l'équipe » qui coche le métier (`?metier=`).
